...
import { getOrdenById } from "@/actions/empresa/ordenes";
...
export function OrderDetailsModal({ orderId, onClose }: OrderDetailsModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-surface/90 backdrop-blur-xl border border-white/[0.07] shadow-2xl rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden animate-in zoom-in-95 fade-in duration-200 flex flex-col">
        {/* Header */}
        <div className="p-6 border-b border-white/[0.05] flex items-center justify-between bg-white/[0.02]">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 uppercase tracking-widest">#{orderId.slice(0, 8).toUpperCase()}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {loading ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="glass-panel p-4 space-y-3">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-400 font-bold text-xs">
                  ...
                  <div className="glass-panel p-4 space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{order.shipping_address?.province}, {order.shipping_address?.zip_code}</p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left text-sm">
                    <thead className="bg-white/[0.02] border-b border-white/[0.05]">
                      <tr>
                        ...
                        <th className="px-4 py-3 font-medium text-slate-500 text-center">Cant.</th>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {order.order_items?.map((item: any) => (
                        <tr key={item.id}>
                          ...
                          <td className="px-4 py-4 text-center text-slate-400">x{item.quantity}</td>
                    ...
                    <tfoot className="bg-white/[0.02] border-t border-white/[0.05]">
                      <tr>
                        <td colSpan={2} className="px-4 py-3 text-slate-400">Envío</td>
                      ...
                      <tr className="border-t border-white/[0.07]">
                        ...
                        <td className="px-4 py-4 text-right text-emerald-400 font-bold text-xl">${new Intl.NumberFormat('es-AR').format(order.total)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
            ...
            <p className="text-center text-slate-500 py-12">No se pudo encontrar la información de la orden.</p>
          )}
        </div>

        {/* Footer */}
        <div className="p-6 bg-black/20 border-t border-white/[0.05] flex justify-end">
          <button 
            onClick={onClose}
