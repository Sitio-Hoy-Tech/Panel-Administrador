...
import { updatePhone, saveShippingZone, deleteShippingZone, type ActionState, type ShippingZone } from "@/actions/empresa/configuracion";
    ...
    <div className="space-y-8">
      ...
      <form action={phoneAction} className="glass-panel p-6 md:p-8 space-y-8">
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            <label htmlFor="phone" className="text-sm font-medium text-slate-300">Número con código de área</label>
            <div className="relative">
              <span className="absolute left-4 top-3.5 text-slate-500 font-medium">+54 9</span>
              <input
                id="phone"
                name="phone"
                type="text"
                placeholder="11 1234 5678"
                className="w-full bg-black/50 border border-white/[0.07] rounded-lg pl-16 pr-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-green-500/50 transition-all placeholder:text-slate-700"
                defaultValue={initialPhone}
                disabled={isPhonePending}
                suppressHydrationWarning
              />
            </div>
            <p className="text-xs text-slate-500">No incluyas el 0 ni el 15. Ej: +54 9 3329 123456</p>
          </div>
        </section>
      ...
      <div className="glass-panel p-6 md:p-8 space-y-4">
        ...
        <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Los pagos se procesan automáticamente. Configuración administrada por SitioHoy.</p>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-6 md:p-8 space-y-6">
        ...
        {zones.length > 0 && (
          <div className="space-y-2">
            {zones.map((zone) => (
              <div key={zone.id} className="flex items-center justify-between p-4 bg-white/[0.02] border border-white/[0.05] rounded-xl group">
                <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="p-1.5 hover:bg-red-500/10 rounded-lg transition-colors text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100"
                    title="Eliminar zona"
                  >
        ...
        <form action={zoneAction} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label htmlFor="zone_name" className="text-sm font-medium text-slate-300">Nombre de la zona</label>
              <input
                id="zone_name"
                name="zone_name"
                type="text"
            ...
            <div className="space-y-2">
              <label htmlFor="zone_price" className="text-sm font-medium text-slate-300">Precio de envío</label>
              <CustomNumberInput 
                id="zone_price"
                name="zone_price"
                min={0}
