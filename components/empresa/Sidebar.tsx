...
import { LayoutDashboard, Package, Settings, LogOut, ExternalLink, HelpCircle, CreditCard, BarChart3, Truck, Tag, Users, Ticket, Plug, Clock, Lock } from "lucide-react";
...
const navigation = [
  ...
  { name: "Analítica Avanzada", href: "/admin/estadisticas", icon: BarChart3 },
        ...
        <div>
          ...
          <ul className="space-y-1.5">
            {secondaryNavigation.map((item) => {
              const locked = expired && !ALLOWED_WHEN_EXPIRED.includes(item.href);
              const active = !locked && isSecondaryActive(item.href);
              const pending = !locked && isPending && pendingHref === item.href;
              return (
                <li key={item.name}>
                  {locked ? (
